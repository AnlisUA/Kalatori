...
use crate::types::{InvoiceWithReceivedAmount, ChainType};
...
#[cfg(test)]
mod tests {
    use crate::types::{Invoice, default_invoice};
    ...
    async fn test_invoice_registry() {
        ...
        assert!(registry.used_asset_ids().await.is_empty());
        ...
        registry.add_invoice(invoice_1.clone()).await;
        assert_eq!(registry.invoices_count().await, 1);
        assert_eq!(registry.get_invoice(&invoice_1_id).await, Some(invoice_1.clone()));
        assert!(registry.get_invoice(&invoice_2_id).await.is_none());
        assert!(registry.get_invoice(&invoice_3_id).await.is_none());
        assert!(registry.get_invoice(&invoice_4_id).await.is_none());
        assert!(registry.get_invoice(&Uuid::new_v4()).await.is_none());

        // Add more invoices, ensure they are all returned
        registry.add_invoices(vec![
            invoice_2.clone(),
            invoice_3.clone(),
            invoice_4.clone(),
            invoice_5.clone(),
        ...
        assert_eq!(registry.get_invoice(&invoice_1_id).await, Some(invoice_1.clone()));
        assert_eq!(registry.get_invoice(&invoice_2_id).await, Some(invoice_2.clone()));
        assert_eq!(registry.get_invoice(&invoice_3_id).await, Some(invoice_3.clone()));
        assert_eq!(registry.get_invoice(&invoice_4_id).await, Some(invoice_4.clone()));
        assert_eq!(registry.get_invoice(&invoice_5_id).await, Some(invoice_5.clone()));
        assert!(registry.get_invoice(&Uuid::new_v4()).await.is_none());

        // Find invoices by address
        let found_1 = registry.find_invoice_by_address("1", ChainType::PolkadotAssetHub, "1").await;
        assert!(found_1.is_some());
        assert_eq!(found_1, Some(invoice_1.clone()));

        let found_2 = registry.find_invoice_by_address("2", ChainType::PolkadotAssetHub, "2").await;
        assert!(found_2.is_some());
        assert_eq!(found_2, Some(invoice_2.clone()));

        let found_3 = registry.find_invoice_by_address("3", ChainType::Polygon, "3").await;
        assert!(found_3.is_some());
        assert_eq!(found_3, Some(invoice_3.clone()));

        let found_4 = registry.find_invoice_by_address("1", ChainType::Polygon, "1").await;
        assert!(found_4.is_some());
        assert_eq!(found_4, Some(invoice_4.clone()));

        let found_5 = registry.find_invoice_by_address("1", ChainType::Polygon, "5").await;
        ...
        registry.update_filled_amount(&invoice_1_id, invoice_1_new_amount).await;

        let invoice_2_new_amount = invoice_2_amount + Decimal::TEN;
        registry.update_filled_amount(&invoice_2_id, invoice_2_new_amount).await;

        let invoice_3_new_amount = invoice_3_amount + Decimal::TEN;
        registry.update_filled_amount(&invoice_3_id, invoice_3_new_amount).await;

        let invoice_4_new_amount = invoice_1_amount + Decimal::TEN;
        registry.update_filled_amount(&invoice_4_id, invoice_4_new_amount).await;

        let invoice_5_new_amount = invoice_1_amount + Decimal::TEN;
        registry.update_filled_amount(&invoice_5_id, invoice_5_new_amount).await;

        let updated_1 = registry.get_invoice(&invoice_1_id).await;
        assert!(updated_1.is_some());
        let updated_1_val = updated_1.unwrap();
        assert_eq!(invoice_1_new_amount, updated_1_val.total_received_amount);
        assert_eq!(invoice_1.invoice, updated_1_val.invoice);

        let updated_2 = registry.get_invoice(&invoice_2_id).await;
        assert!(updated_2.is_some());
        let updated_2_val = updated_2.unwrap();
        assert_eq!(invoice_2_new_amount, updated_2_val.total_received_amount);
        assert_eq!(invoice_2.invoice, updated_2_val.invoice);

        let updated_3 = registry.get_invoice(&invoice_3_id).await;
        assert!(updated_3.is_some());
        let updated_3_val = updated_3.unwrap();
        assert_eq!(invoice_3_new_amount, updated_3_val.total_received_amount);
        assert_eq!(invoice_3.invoice, updated_3_val.invoice);

        let updated_4 = registry.get_invoice(&invoice_4_id).await;
        assert!(updated_4.is_some());
        let updated_4_val = updated_4.unwrap();
        assert_eq!(invoice_4_new_amount, updated_4_val.total_received_amount);
        assert_eq!(invoice_4.invoice, updated_4_val.invoice);

        let updated_5 = registry.get_invoice(&invoice_5_id).await;
        assert!(updated_5.is_some());
        let updated_5_val = updated_5.unwrap();
        assert_eq!(invoice_5_new_amount, updated_5_val.total_received_amount);
        assert_eq!(invoice_5.invoice, updated_5_val.invoice);

        // Remove single invoice
        registry.remove_invoice(&invoice_1_id).await;

        assert_eq!(registry.invoices_count().await, 4);
        assert!(registry.get_invoice(&invoice_1_id).await.is_none());
        assert!(registry.get_invoice(&invoice_2_id).await.is_some());
        assert!(registry.get_invoice(&invoice_3_id).await.is_some());
        assert!(registry.get_invoice(&invoice_4_id).await.is_some());
        assert!(registry.get_invoice(&invoice_5_id).await.is_some());

        // Remove multiple invocies
        registry.remove_invoices(&[
